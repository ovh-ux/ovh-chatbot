...
function validatePayloadSignature (secret, buf, signatureHeader, payloadEncoding, req) {
  if (!signatureHeader) {
    ...
    const signatureSplit = signatureHeader.split("=");
    const hmacType = signatureSplit[0];
    const signatureHash = signatureSplit[1];
...
module.exports = (config) => function verifyRequestSignature (req, res, buf) {
  ...
  case `${config.server.basePath}/webhook`: {
    ...
    //  *
    return validatePayloadSignature(config.facebook.appSecret, buf, req.headers["x-hub-signature"], "hex", req);
  }
  case `${config.server.basePath}/twitter`: {
    ...
    //  *
    return validatePayloadSignature(config.twitter.apiSecret, buf, req.headers["x-twitter-webhooks-signature"], "base64", req);
  }
  default: {
    return null;
  }
