  ...
  return SlackModel.findOne({ team_id })
    ...
    .then((slackInfos) => {
      ...
      slack.sendButtonMessage = (channel, messageData, deleteOriginal = false) => {
        ...
        if (deleteOriginal) {
          delete params.replace_original;
          delete params.delete_original;
        }
      ...
      slack.send = (channel, message) => {
        ...
        if (message instanceof ButtonsListMessage || message instanceof ButtonsMessage) {
          let messageData = buttonsMessageAdapter(message);
          let promise;
          ...
          return Bluebird.all([promise, slack.sendButtonMessage(channel, messageData.attachments, message.delete_original)]);
        }
