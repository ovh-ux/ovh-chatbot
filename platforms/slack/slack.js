...
function getSlackApi (team_id) {
  return SlackModel.findOne({ team_id }).exec().then((slackInfos) => {
    ...
    slack.send = (channel, message, message_ts = null, locale) => {
      if (typeof message === "string" || message instanceof TextMessage) {
        return slack.apiAsync(message_ts != null ? UPDATE_MESSAGE : POST_MESSAGE, textMessageAdapter(channel, message, message_ts, locale));
      }

      if (message instanceof ButtonsListMessage || message instanceof ButtonsMessage) {
        return slack.apiAsync(message_ts != null ? UPDATE_MESSAGE : POST_MESSAGE, buttonsMessageAdapter(channel, message, message_ts, locale));
      }
